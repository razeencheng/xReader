...
describe('broadcast', () => {
  ...
  describe('local-echo vs both-field poll snapshot', () => {
    it('consumeLocalBroadcast matches a both-field poll echo of a single-field local read', async () => {
      const { broadcast, consumeLocalBroadcast, close } = await import('./broadcast');

      broadcast({ type: 'state-change', articleId: 101, is_read: true });
      expect(
        consumeLocalBroadcast({ articleId: 101, is_read: true, is_starred: false }),
      ).toBe(true);
      // consumed once: a second identical poll is NOT suppressed
      expect(
        consumeLocalBroadcast({ articleId: 101, is_read: true, is_starred: false }),
      ).toBe(false);
    ...
    it('consumeLocalBroadcast matches a both-field poll echo of a single-field local star', async () => {
      const { broadcast, consumeLocalBroadcast, close } = await import('./broadcast');

      broadcast({ type: 'state-change', articleId: 102, is_starred: true });
      expect(
        consumeLocalBroadcast({ articleId: 102, is_read: false, is_starred: true }),
      ).toBe(true);

      close();
    });

    it('consumeLocalBroadcast does NOT suppress a genuine change that disagrees on the asserted field', async () => {
      const { broadcast, consumeLocalBroadcast, close } = await import('./broadcast');

      broadcast({ type: 'state-change', articleId: 103, is_read: true });
      // poll says is_read:false for 103 — this is NOT the local change; must not be consumed
      expect(
        consumeLocalBroadcast({ articleId: 103, is_read: false, is_starred: false }),
      ).toBe(false);

      close();
    });

    it('consumeLocalBroadcast does not match a different article', async () => {
      const { broadcast, consumeLocalBroadcast, close } = await import('./broadcast');

      broadcast({ type: 'state-change', articleId: 104, is_read: true });
      expect(
        consumeLocalBroadcast({ articleId: 999, is_read: true, is_starred: false }),
      ).toBe(false);

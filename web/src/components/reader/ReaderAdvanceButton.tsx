...
export function ReaderAdvanceButton({ mode, phase, hidden, onAdvance }: Props) {
  ...
  return (
    <button
      ...
      disabled={busy}
      onClick={onAdvance}
      className="fixed z-[55] inline-flex min-h-12 min-w-12 items-center justify-center rounded-full border border-[var(--border-strong)] bg-[var(--bg-elevated)] px-4 text-[13px] font-semibold text-[var(--text-2)] shadow-[0_10px_30px_rgba(30,24,16,0.16)] transition-opacity motion-reduce:transition-none disabled:cursor-wait disabled:opacity-65"
      style={{
